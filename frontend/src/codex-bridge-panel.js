...
template.innerHTML = `
  <style>
    ...
    * {
    ...
    .desktop-feature-spinner {
      width: 32px;
      height: 32px;
      border: 3px solid color-mix(in srgb, var(--accent-color) 18%, var(--border-color) 82%);
      border-top-color: var(--accent-color);
      border-radius: 50%;
      animation: codex-spin 900ms linear infinite;
    }
    ...
    .project-secondary-actions {
      background: transparent;
      border-left-color: color-mix(in srgb, var(--accent-color) 24%, var(--border-color) 76%);
    }

    .project-secondary-actions .rail-menu-item:hover,
    .project-secondary-actions .rail-menu-item:focus-visible {
...
class CodexBridgePanel extends HTMLElement {
  ...
  set hass(value) {
    ...
    // HA publishes a new hass object for ordinary entity changes. This panel
    // does not render those entities, so rebuilding its controls loses hover,
    // focus and open popovers without changing anything visible.
    if (previous && value
      && previous.user?.id === value?.user?.id
      && previous.config?.time_zone === value?.config?.time_zone) return;
    this._render();
  }
  ...
  _installStaticUi() {
    ...
    this._mobileDrawerMedia.addEventListener("change", this._mobileDrawerMediaListener);
    this._mobileDrawerMediaListening = true;
    this._contextDrawerMedia = typeof window.matchMedia === "function"
      ? window.matchMedia("(max-width: 880px), (min-width: 1121px) and (max-width: 1480px)")
      : {
        matches: false,
        addEventListener() {},
        removeEventListener() {},
    ...
    this._contextDrawerMedia.addEventListener("change", this._contextDrawerMediaListener);
    this._contextDrawerMediaListening = true;
    this._timelineCompactMedia = typeof window.matchMedia === "function"
  ...
  _toggleMobileDrawer(drawer, trigger) {
    const enabled = drawer === "navigation"
      ? this._mobileDrawerMedia?.matches
      : this._contextDrawerMedia?.matches;
    if (!enabled) {
      return;
    }
  ...
  _syncMobileDrawer() {
    ...
    if (
      (this._mobileDrawer === "navigation" && !mobileNavigation)
      || (this._mobileDrawer === "context" && !contextDrawer)
    ) {
      this._mobileDrawer = null;
      this._mobileDrawerReturnFocus = null;
    }
  ...
  async _saveAccountProfile() {
    ...
    const input = this.shadowRoot.getElementById("account-profile-label");
  ...
  async _switchAccountProfile(profileId) {
    ...
    try {
      ...
      this._accountProfileFeedback = error?.code === "account_profile_reauthentication_required"
        ? `${profile.label} needs a fresh sign-in. Use Add another account, sign in to it, then save it again.`
        : "The switch could not be verified. Refresh the account list before trying again.";
    } finally {
      this._accountProfilePending = false;
      this._renderAppMenu();
    }
  ...
  async _handleDesktopAction(action, dataset = {}, target, { confirmed = false } = {}) {
    ...
    if (action === "open-schedule-description") { this._clearDesktopFormDraft(state); state.editingAutomation = null; state.scheduleContext = this._scheduleContext(); state.form = "schedule-description"; }
      ...
      try {
        ...
        // Reading history also refreshes the definition changed by HA's scheduler.
        // The existing destination refresh preserves an open form and its draft.
  ...
  _isRefreshLockTarget(target) {
    ...
    if (
      target.closest("[data-interaction-id]")
  ...
  _renderAssistantSection() {
    const section = this.shadowRoot.getElementById("assistant-section");
  ...
  _renderStatusBanner() {
    ...
    if (this._renderedStatusBannerKey === renderKey && banner.classList.contains("visible")) {
      for (const button of banner.querySelectorAll(".banner-action")) {
        button.disabled = this._authActionPending && AUTH_ACTION_IDS.has(button.dataset.action);
      }
      return;
    }
  ...
  _renderRunActivity() {
    ...
    const activity = this._runActivityForThread();
    const started = activity.busy ? this._events.slice().reverse().find((event) => event.event_type === "run.started"
      && (!activity.runId || event.payload?.run_id === activity.runId)) : null;
    ...
    if (previousWrap && nextWrap && previousThreadId === this._renderedRunActivityThreadId) {
      ...
      const desiredChildren = [...nextTooltip.children].map((child) => {
        ...
        if (child.classList.contains("run-command-details")) {
          ...
          const existing = previousDetails.find((details) =>
            !reusedDetails.has(details) && details.querySelector("pre")?.textContent === preview);
          if (existing) {
  ...
  _scheduleArtifactRefreshRetry(threadId = this._selectedThreadId) {
    ...
    const delay = ARTIFACT_REFRESH_RETRY_DELAYS_MS[Math.min(attempt, ARTIFACT_REFRESH_RETRY_DELAYS_MS.length - 1)];
    const generation = this._artifactRefreshRetryGeneration;
    this._artifactRefreshRetryTimer = window.setTimeout(() => {
      this._artifactRefreshRetryTimer = null;
      if (
        generation !== this._artifactRefreshRetryGeneration
        || threadId !== this._selectedThreadId
        || !this.isConnected
      ) {
        return;
          ...
          : section.id === "background"
            ...
            : section.id === "browser"
              ...
              : section.id === "sources"
                ...
                : false;
      if (["background", "browser", "sources"].includes(section.id) && !hasDetails) {
        continue;
      }
      const heading = document.createElement("div");
  ...
  async _createWorkspaceArchive({ retry = false } = {}) {
    ...
    const isCurrent = () => (
      threadId === this._selectedThreadId
      && selectionEpoch === this._threadSelectionEpoch
      && this.isConnected
    );
  ...
  _syncSelectedArtifact() {
    ...
    const previewCandidate =
      this._artifacts.find((artifact) => isAutoPreviewCandidate(artifact))
  ...
  _directThreads(includeArchived) {
    return this._threads.filter(
      (thread) =>
        ...
        (includeArchived || !this._chatContextMenu.isGrouped(thread)) &&
        this._threadMatchesQuery(thread)
    );
  }
