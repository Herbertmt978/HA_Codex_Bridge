...
- **Evidence refs:** `90-evidence.md` preserves the bounded live-accepted `0.6.5` evidence. App/Integration `0.6.6` is published and signed at image digest `sha256:aab2882333a70354624c5ec3a461f738f5a3495ab5340b3161f4e941c6fe4767`, but has no recorded target-HA live acceptance. No immutable `0.7.0` image digest or live-acceptance evidence exists yet.
- **Release state:** `0.6.6` is the published/signed baseline. The expanded capability set is release candidate `0.7.0` with Bridge `0.6.0`; it requires fresh CI, image-signing, and live-acceptance evidence.
- **Blocked on:** Linux CI, publication/signing and target-Home-Assistant acceptance for `0.7.0`, plus the existing external Nabu Casa/Cloudflare route, cold restore, first unattended App-update canary, and previous-image recovery evidence gaps.
- **Next step:** commit and review the candidate, merge after green CI, publish/sign App `0.7.0`, then install and exercise chat plus each capability on the target Home Assistant.
...
- **Branch:** `Herb/0.6.7-live-activity`
- **Integrated main head:** `2d16085` (release `0.6.6`).
- **Worktree status at checkpoint:** The expanded backend, Integration, frontend, release projections, public documentation, and repository `AGENTS.md` are intentionally uncommitted after local release verification and pending final review/CI. The original main worktree's unrelated user edits remain untouched.
- **Current focused matrix:** Integration `0.7.0`, App `0.7.0`, Bridge `0.6.0`, and Codex `0.144.4` are being shipped. Publication, signing, and live acceptance are pending.
...
- **Release discipline:** App images remain immutable. `0.6.6` is signed/published and `0.6.5` is live-accepted within the recorded boundaries; `0.7.0` needs fresh release evidence. Supervisor arbitrary prior-image selection is not validated. Cold restore, first automatic update, previous-image recovery, and external blocked-network routing remain open evidence.
- **Decision:** retain the bounded historical acceptance records and complete `0.7.0` acceptance plus the four broader recovery gates separately.
