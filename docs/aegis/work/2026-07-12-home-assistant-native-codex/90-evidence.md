...
| Owner-aware catalogues | The private owner marker participates in the settled model/capability cache identity. A same-plan account switch invalidates both shared catalogues once, discovers the new account's model and reasoning defaults, and keeps the marker out of every public projection. |
...
| Atomic automation target admission | A broker-owned one-shot admission reserves the exact prompt lease and an idempotency slot before an automation creates or changes its target chat, then transfers the same lease into submission. Deterministic inverse races prove logout winning leaves the chat untouched and automation winning blocks logout until admission; abandoned, duplicate, mismatched, capacity-exhausted, queued, and exact-replay paths neither leak capacity nor mutate a second target. |
...
| Public thread boundary | Thread list/detail/create/update/archive/restore return a dedicated browser-safe DTO. Provider session/thread/turn handles, active runtime ownership, and queued-prompt internals remain private; all local chat/project/workspace/file/model/archive metadata remains public. The Integration strips the same fields defensively when paired temporarily with an older App. |
| Public interaction and event boundary | Approvals and user questions are addressed by an App-local `interaction_id`; provider run, turn, and item locators remain inside the App and are resolved only after local thread validation. `interactions_v2` makes the contract explicit across App/Integration version skew. Fresh events omit provider-turn continuity, and every replay/wait/SSE read projects historical journals through the current public schema so legacy fields cannot reappear in the browser. |
| TDD evidence | Owner-marker/coordinator tests first failed on the missing helper/callback; storage tests first failed on the missing binding method; lifecycle composition first failed because the legacy provider handle remained attached; recovered-checkpoint, in-flight notification, active-login-poll, queued-promotion, missing-workspace, stale-login-generation, active-steer, blocked-poll, same-plan cache, automation target-race, replay-target, public-thread, and public-interaction/event tests then failed on stale startup/auth/admission, retained leases, duplicate events, stale catalogues, premature local mutation, or private browser fields. Each focused slice passed after its minimal repair. |
| Complete Bridge suite | Final Windows/Python 3.14 plugin-independent matrix after all review-race, admission, and public-projection repairs: `1496 passed, 218 skipped` in 299.39 seconds. Repository-wide Ruff, compileall, release synchronization, Codex-lock, and diff hygiene passed. |
| Focused final-hardening matrix | The final account-neutral/runtime/privacy review slice passed `295 passed, 3 skipped`; the complete Runtime Broker suite independently passed `195 passed, 3 skipped`. These cover startup order, authoritative updates, in-flight invalidation, identity-less and transient states, queue promotion, atomic provider continuity, automation target admission/replay, lease cleanup/capacity, deletion-safe lock order, missing historical workspaces, stale login generations, active steers, blocked-poll dedupe, owner-aware catalogues, browser-safe threads/interactions, and historical-event redaction. |
